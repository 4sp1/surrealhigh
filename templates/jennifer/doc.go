...
func NewField(name, t string) DocField {
	return DocField{Field: sh.Field(name), t: t}
}
...
func NewDoc(pkg sh.Package, table sh.Table, fields ...DocField) (doc Doc) {
	...
	for _, field := range fields {
		f.Type().Id(field.docStructFieldTypeId(doc)).Id(field.t)
	}
